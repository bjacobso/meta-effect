...
import { Effect, Schema as S, Array, pipe } from "effect"
import { Command, Options, Args } from "@effect/cli"
import { NodeRuntime, NodeContext, NodeFileSystem } from "@effect/platform-node"
import * as Path from "node:path"

import { Git, GH, Claude } from "./shell-runner"
import * as T from "./transforms"
import { ReleaseJSON, type Commit, type PR } from "./types"
...
export const runPlan = (plan: ReleasePlan, dryRun = false) =>
  Effect.gen(function* () {
    ...
    if (dryRun) {
      ...
      yield* Effect.log(`Markdown preview (first 800 chars):\n${md.slice(0, 800)}...`)
    ...
    if (plan.output.toJsonFile) {
      yield* fs.writeFileString(plan.output.toJsonFile, JSON.stringify(limited, null, 2))
...
const runCommand = Command.make(
  "run",
  { options: Options.all({ since: sinceOption, until: untilOption, dryRun: dryRunOption }) },
  ({ options }) =>
    Effect.gen(function* () {
      const plan = weeklyPlan
      const window = options.since && options.until
        ? { kind: "sinceUntil" as const, since: options.since, until: options.until }
        : plan.window
...
const emitWorkflowCommand = Command.make(
  "emit-workflow",
  {},
  () => Effect.sync(() => {
...
const cli = Command.make("release-plan")
  .pipe(Command.withSubcommands([runCommand, emitWorkflowCommand]))
